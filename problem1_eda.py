...
# 聚合数据集
# 每小时平均（区分工作日/休息日）
hourly_avg = df.groupby(['是否为工作日', '小时(0-23)'])[['借出量', '归还量']].mean().reset_index()  # 别忘了reset_index把分组索引变回普通列
hourly_avg = hourly_avg.rename(columns={'小时(0-23)': '小时'})

# 每个站点的7天总借出量（降序）
station_total_b = df.groupby(['站点编号', '站点名称', '经度', '纬度'])['借出量'].sum().reset_index()
station_total_b = station_total_b.sort_values('借出量', ascending=False)

# 每个站点的7天总归还量（降序）
station_total_r = df.groupby(['站点编号', '站点名称', '经度', '纬度'])['归还量'].sum().reset_index()
station_total_r = station_total_r.sort_values('归还量', ascending=False)
...
# 输出结果
# （1）高峰时段
print("(1) 全天典型高峰时段及持续时长：")
weekday_borrow = hourly_avg[hourly_avg['是否为工作日'] == 1]
weekend_borrow = hourly_avg[hourly_avg['是否为工作日'] == 0]

# 高于日均的1.5倍
daily_mean_borrow = weekday_borrow['借出量'].mean()
peak_borrow_hours = weekday_borrow[weekday_borrow['借出量'] > 1.5 * daily_mean_borrow]['小时'].values
print(f"工作日借出高峰：{list(peak_borrow_hours)} 时，持续约 {len(peak_borrow_hours)} 小时")

daily_mean_return = weekday_borrow['归还量'].mean()
peak_return_hours = weekday_borrow[weekday_borrow['归还量'] > 1.5 * daily_mean_return]['小时'].values
print(f"工作日归还高峰：{list(peak_return_hours)} 时，持续约 {len(peak_return_hours)} 小时")
...
# （3）工作日 vs 休息日差异
weekday_total_b = df[df['是否为工作日']==1]['借出量'].sum()
weekend_total_b = df[df['是否为工作日']==0]['借出量'].sum()
print(f"\n(3) 工作日总借出量：{weekday_total_b}，休息日总借出量：{weekend_total_b}，工作日是休息日的 {weekday_total_b/weekend_total_b:.2f} 倍")

weekday_total_r = df[df['是否为工作日']==1]['归还量'].sum()
weekend_total_r = df[df['是否为工作日']==0]['归还量'].sum()
...
# 工作日借出（实线）
plt.plot(weekday_borrow['小时'], weekday_borrow['借出量'], 'r-o', linewidth=1.5, markersize=6, label='工作日借出')
# 工作日归还（虚线）
plt.plot(weekday_borrow['小时'], weekday_borrow['归还量'], 'r-.s', linewidth=1.5, markersize=6, label='工作日归还')
# 休息日借出（实线）
plt.plot(weekend_borrow['小时'], weekend_borrow['借出量'], 'c-o', linewidth=1.5, markersize=6, label='休息日借出')
# 休息日归还（虚线）
plt.plot(weekend_borrow['小时'], weekend_borrow['归还量'], 'c-.s', linewidth=1.5, markersize=6, label='休息日归还')
plt.title('工作日与休息日借还时段对比图', fontsize=14)
plt.xlabel('小时 (0-23)', fontsize=12)
plt.ylabel('平均车辆数', fontsize=12)
plt.legend(loc='best')
plt.grid(True, alpha=0.3)
plt.savefig('Q1_工作日与休息日借还时段对比图.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('Q1_站点总借出量柱状图.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.ylabel('站点名称')
plt.tight_layout()
plt.savefig('Q1_借出量热力图.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.ylabel('站点名称')
plt.tight_layout()
plt.savefig('Q1_归还量热力图.png', dpi=300, bbox_inches='tight')
plt.close()

# 图5：空间散点图（早高峰8点）
plt.figure(figsize=(10, 8))
plt.scatter(station_total_b['经度'], station_total_b['纬度'], s=station_total_b['借出量']*0.5, c=station_total_b['借出量'], cmap='YlOrRd', alpha=0.7)
...
plt.ylabel('纬度')
plt.grid(True)
plt.savefig('Q1_空间散点图_早高峰.png', dpi=300, bbox_inches='tight')
plt.close()

# 图6.1：总借出量Top5站点的每小时平均借出量曲线（这里先不区分工作日/休息日）
plt.figure(figsize=(14, 8))
top5_stations_b = station_total_b.head(5)['站点编号'].tolist()
print(f"借出量Top5站点：{station_total_b.head(5)[['站点名称', '借出量']].to_string(index=False)}")
df_top5 = df[df['站点编号'].isin(top5_stations_b)]
hourly_top5_b = df_top5.groupby(['站点名称', '小时(0-23)'])['借出量'].mean().reset_index()
...
plt.tight_layout()
plt.savefig('Q1_Top5站点每小时借出量曲线.png', dpi=300, bbox_inches='tight')
plt.close()

# 图6.2：总借出量Top5站点的每小时平均归还量曲线（这里先不区分工作日/休息日）
plt.figure(figsize=(14, 8))
hourly_top5_r = df_top5.groupby(['站点名称', '小时(0-23)'])['归还量'].mean().reset_index()
...
plt.xticks(range(0, 24, 2))  # 每2小时显示一个刻度
plt.tight_layout()
plt.savefig('Q1_Top5站点每小时归还量曲线.png', dpi=300, bbox_inches='tight')
plt.close()

# 图6.3：总借出量Bottom5站点的每小时平均借出量曲线（这里先不区分工作日/休息日）
plt.figure(figsize=(14, 8))
bottom5_stations_b = station_total_b.tail(5)['站点编号'].tolist()
print(f"借出量Bottom5站点：{station_total_b.tail(5)[['站点名称', '借出量']].to_string(index=False)}")
df_bottom5 = df[df['站点编号'].isin(bottom5_stations_b)]
hourly_bottom5_b = df_bottom5.groupby(['站点名称', '小时(0-23)'])['借出量'].mean().reset_index()
...
plt.xticks(range(0, 24, 2))  # 每2小时显示一个刻度
plt.tight_layout()
plt.savefig('Q1_Bottom5站点每小时借出量曲线.png', dpi=300, bbox_inches='tight')
plt.close()

# 图6.4：总借出量Bottom5站点的每小时平均归还量曲线（这里先不区分工作日/休息日）
plt.figure(figsize=(14, 8))
hourly_bottom5_r = df_bottom5.groupby(['站点名称', '小时(0-23)'])['归还量'].mean().reset_index()
...
plt.xticks(range(0, 24, 2))  # 每2小时显示一个刻度
plt.tight_layout()
plt.savefig('Q1_Bottom5站点每小时归还量曲线.png', dpi=300, bbox_inches='tight')
plt.close()

# 以下是为第一题第3问新写的部分，之后可以跟前面整合一下
print("\n=== 开始生成工作日 vs 休息日差异可视化 ===")
...
type_work = df[df['是否为工作日']==1].groupby('站点类型')['借出量'].mean()
type_weekend = df[df['是否为工作日']==0].groupby('站点类型')['借出量'].mean()
...
totals = pd.DataFrame({
    '日类型': ['工作日', '休息日'],
    '总借出量': [weekday_df['借出量'].sum(), weekend_df['借出量'].sum()]
})
...
# 图B: 空间差异散点图（工作日 vs 休息日借出量，点大小=差异）
import matplotlib.pyplot as plt

# ====================== 图B: 工作日 vs 休息日 空间差异散点图 ======================
plt.figure(figsize=(14, 6))

# 1. 计算每个站点的日均借出量（工作日）
weekday_station_avg = (weekday_df.groupby(['站点编号', '站点名称', '经度', '纬度'])['借出量']
                       .sum() / 5)          # 假设有5个工作日，根据实际天数调整
weekday_station_avg = weekday_station_avg.reset_index(name='日均借出量')

# 2. 计算每个站点的日均借出量（休息日）
weekend_station_avg = (weekend_df.groupby(['站点编号', '站点名称', '经度', '纬度'])['借出量']
                       .sum() / 2)          # 假设有2个休息日，根据实际天数调整
weekend_station_avg = weekend_station_avg.reset_index(name='日均借出量')
...
# 图B(2)：空间差异图（合并）
## 合并两个平均值
merged = weekday_station_avg.merge(weekend_station_avg, 
                                  on=['站点编号', '站点名称', '经度', '纬度'], 
                                  suffixes=('_work', '_weekend'))

merged['差异'] = merged['日均借出量_work'] - merged['日均借出量_weekend']

plt.figure(figsize=(8, 6))
sc = plt.scatter(merged['经度'], merged['纬度'], 
                 s=merged['差异'].abs() * 2,      # 差异越大点越大
                 c=merged['差异'],                # 正负用颜色区分
                 cmap='RdBu_r', vmin=-max(abs(merged['差异'])), vmax=max(abs(merged['差异'])),
...
# 图C: 站点借出量差异热力图（工作日-休息日）
diff_heatmap = station_diff.set_index('站点名称')[['差异']].T
plt.figure(figsize=(14, 8))
sns.heatmap(diff_heatmap, cmap='RdBu', center=0, annot=True, fmt='.1f', linewidths=0.5)
...
# 图E: 净归还量差异时序图
plt.figure(figsize=(10, 6))
plt.plot(weekday_hourly.index, weekday_hourly['净归还量'], label='工作日净归还量', marker='o', color='red')
plt.plot(weekend_hourly.index, weekend_hourly['净归还量'], label='休息日净归还量', marker='s', color='blue')
plt.axhline(0, color='gray', linestyle='--')
plt.title('工作日与休息日净归还量对比（净归还量 = 归还量 - 借出量）')
plt.xlabel('小时'); plt.ylabel('平均净归还量')
plt.legend()
plt.grid(True)
...
# 图G: 借还平衡差异（借出/归还比例）
ratio_work = weekday_df['借出量'].sum() / weekday_df['归还量'].sum()
ratio_weekend = weekend_df['借出量'].sum() / weekend_df['归还量'].sum()
plt.figure(figsize=(6, 6))
plt.bar(['工作日', '休息日'], [ratio_work, ratio_weekend], color=['#FF6B6B', '#4ECDC4'])
