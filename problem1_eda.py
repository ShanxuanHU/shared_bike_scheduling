...
# 聚合数据集
# 每小时平均（区分工作日/休息日，不区分站点）
hourly_avg = df.groupby(['是否为工作日', '小时(0-23)'])[['借出量', '归还量']
                                                ].mean().reset_index()  # 别忘了reset_index把分组索引变回普通列
weekday_hourly_avg = hourly_avg[hourly_avg['是否为工作日'] == 1]
weekend_hourly_avg = hourly_avg[hourly_avg['是否为工作日'] == 0]

# 每个站点的7天总借出量与归还量（降序）
station_total_b = df.groupby(['站点编号', '站点名称', '经度', '纬度'])[
    '借出量'].sum().reset_index()
station_total_b = station_total_b.sort_values('借出量', ascending=False)
station_total_r = df.groupby(['站点编号', '站点名称', '经度', '纬度'])[
    '归还量'].sum().reset_index()
station_total_r = station_total_r.sort_values('归还量', ascending=False)

# 站点×小时 借出量与归还量 热力矩阵（待会热力图用）
